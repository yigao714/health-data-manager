...
本版本新增**外部 CSV 数据导入**与**数据表内联编辑**两大能力，打通"导入历史数据 → 核对 → 就地修正"的闭环。所有改动均经真实数据后端测试与真实浏览器端到端验证。

### 🆕 新功能
- **[NEW] 外部 CSV 数据导入**：截图识别页新增「📄 外部 CSV 数据导入」卡片，可导入其他设备/App 导出的健康 CSV，增量合并到指定成员。
  - **按列名语义识别**字段（维护别名表，不依赖列顺序；未来数据列更多/顺序不同也能认）；识别不出的列如实列出供核对，绝不乱填或丢弃。
  - 自动探测 **UTF-8 / GBK** 编码；统一多种日期格式（`M/D/YYYY`、`YYYY-MM-DD` 等）；自动将"心率区间 51-92"拆分为最低/最高心率。
  - CSV 内重复日期按后值覆盖聚合；并入库时仅填补已有记录的空字段（不覆盖既有、不漏不添）。
  - 如实导入真实数据，对单日距离过大等可疑值在导入摘要中**标记提示核对**，不擅自修改或丢弃。
- **[NEW] 数据表内联编辑**：数据管理表每行新增「✏️ 编辑」，可逐行就地修正步数、距离、心率最低/最高、静息心率、血氧最低/最高、睡眠时长、入睡/醒来、睡眠分数。
  - **仅更新改动字段**，未在表格展示的字段（睡眠分期、压力、呼吸等）保存时原样保留。
  - 空值表示清空（缺失合法）；保存前走医学/物理**硬拦截**，挡下不可能值（如血氧 > 100%）并提示原因，绝不写入坏数据；保存后自动重生成仪表盘。

### 🔧 改进
- **[IMPROVE] 数据表新增日期键行定位**：为内联编辑提供稳定的行标识与"取消即还原"。
